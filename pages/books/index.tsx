...
export default function Books() {
  ...
  const [sortBy, setSortBy] = useState<'title' | 'author' | 'year' | 'price'>('title')
  const [alert, setAlert] = useState<Alert | null>(null)
  const [showLoginModal, setShowLoginModal] = useState(false)
  const [connectionStatus, setConnectionStatus] = useState<'testing' | 'connected' | 'failed'>('testing')

  // Тестування підключення при завантаженні
  useEffect(() => {
  ...
  const filterAndSortBooks = () => {
    let filtered = books.filter(book => {
      ...
      const matchesStatus = statusFilter === 'all' || 
                            ...
                            (statusFilter === 'out_of_stock' && book.status === 'out_of_stock')
  ...
  const ConnectionStatus = () => {
    const statusConfig = {
      testing: { text: '🔧 Перевірка підключення...', style: 'bg-yellow-100 text-yellow-800' },
      connected: { text: '✅ Підключено до API', style: 'bg-green-100 text-green-800' },
      failed: { text: '❌ Немає підключення', style: 'bg-red-100 text-red-800' }
    }
    
    const config = statusConfig[connectionStatus]
    
    return (
      <div className={`text-sm font-medium px-3 py-1 rounded-full ${config.style}`}>
        {config.text}
      </div>
    )
  }
  ...
  const DebugPanel = () => (
    <div className="bg-gray-100 p-4 rounded-lg mb-4">
      <h3 className="font-bold mb-2">🔧 Діагностика підключення</h3>
      ...
      <div className="mt-2 text-sm text-gray-600">
        <p>URL: {process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}</p>
        ...
        <div className="container mx-auto">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-6">
            <div className="flex items-center mb-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-red-800">Помилка завантаження</h2>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {showLoginModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 relative">
              ...
              <LoginForm onSuccess={handleLoginSuccess} onClose={handleCloseLoginModal} />
            </div>
          </div>
        )}
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Відкрийте для себе світ чудових книг від класики до сучасних бестселерів
          </p>
        </div>
        ...
        {!loading && books.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm text-center">
              ...
              <div className="text-gray-600">Доступно для читання</div>
            </div>
            <div className="bg-white rounded-lg p-6 shadow-sm text-center">
              <div className="text-3xl font-bold text-purple-600 mb-2">
                {Math.round(books.reduce((sum, book) => sum + book.pages, 0) / books.length)}
              </div>
              <div className="text-gray-600">Середня кількість сторінок</div>
            </div>
          </div>
        )}
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="mt-4 text-gray-600 text-lg">Завантажуємо колекцію книг...</p>
          </div>
        ) : (
          <>
                ...
                <div key={book.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col h-full">
                  ...
                  <div className="relative pb-[60%] bg-gray-200 flex-shrink-0">
                    ...
                    <div className="absolute top-2 right-2">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${getBookStatus(book).styles}`}>
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    <div className="flex-grow min-h-0">
                      ...
                      <p className="text-gray-600 mb-2 break-words">{book.author}</p>
            ...
            {filteredBooks.length === 0 && !loading && (
              <div className="text-center py-20 bg-white rounded-xl shadow-lg">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-4">Книг не знайдено</h3>
